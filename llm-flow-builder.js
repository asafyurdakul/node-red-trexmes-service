...
module.exports = function (RED) {
  ...
  function LLMFlowBuilder(config) {
    ...
    function buildSystemPrompt() {
      const extraInstructions = getEffectiveSystemPrompt();

      return `You are an expert Node-RED flow architect.
Your ONLY job is to return a valid Node-RED flow as a JSON array.

STRICT RULES:
1. Respond with ONLY a raw JSON array — no markdown, no explanation, no code fences.
2. Every node must have: id, type, x, y, wires, z (tab id).
3. Use a tab node (type: "tab") as the first element, assign its id to every node's "z" field.
4. Ensure all node connections in "wires" reference valid node ids.
5. For inject nodes, set "once": false unless the user says otherwise.
6. Do not include any text outside the JSON array.

Example minimal structure:
[
  {"id":"tab1","type":"tab","label":"My Flow","disabled":false,"info":""},
  {"id":"n1","type":"inject","z":"tab1","name":"Start","props":[],"repeat":"","crontab":"","once":false,"onceDelay":0.1,"topic":"","payload":"","payloadType":"date","x":150,"y":100,"wires":[["n2"]]},
  {"id":"n2","type":"debug","z":"tab1","name":"Output","active":true,"tosidebar":true,"console":false,"tostatus":false,"complete":"payload","targetType":"msg","statusVal":"","statusType":"auto","x":350,"y":100,"wires":[]}
]
${extraInstructions ? '\n--- Geliştirici Talimatları ---\n' + extraInstructions : ''}`;
    ...
    // ── Retrieve Node-RED admin credentials ───────────────────────────────
    async function getNRAdminToken() {
      const settings = RED.settings;
      const adminAuth = settings.adminAuth;
      if (!adminAuth) return null; // no auth configured

      try {
        const http = require('http');
        const qs   = require('querystring');

        // Try default admin credentials — user can customise
    ...
    async function callLLM(prompt) {
      ...
      if (provider === 'openai' || provider === 'deepseek' || provider === 'custom') {
        ...
        // Google Gemini — model adını dinamik olarak URL'e yaz
        finalUrl = new URL(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent`);
    ...
    node.on('input', async function (msg, send, done) {
      ...
      try {
        ...
        msg.generatedFlow = flowArray;

        // 3. Import into Node-RED (optional)
        if (node.autoImport) {
          try {
            status.working('Flow import ediliyor...');
            const tabLabel = await importFlow(flowArray);
